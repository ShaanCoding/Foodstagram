...
async function GetBusinessPosts(req: Request, res: Response) {
  ...
  if (account === undefined) {
    ...
    for (let i = 0; i < posts.length; i++) {
      const postImages = (await Query(postImageQuery, [
        posts[i].post_id.toString(),
      ])) as any;
      posts[i].post_image = postImages[0].image_url;
      const numberOfComments = (await Query(numberOfCommentsOnPostQuery, [
        posts[i].post_id.toString(),
      ])) as any;
      posts[i].commentsCount = numberOfComments[0]["COUNT(*)"];
      const numberOfLikes = (await Query(numberOfLikesQuery, [
        posts[i].post_id.toString(),
      ])) as any;
...
async function GetIndividualBusinessPost(req: Request, res: Response) {
  ...
  if (post_id === undefined) {
    ...
    const post = (await Query(individualPostQuery, [post_id])) as any;
