...
async function Block(req: Request, res: Response) {
	...
	const follow_row = (await Query(checkBlockQuery, [
		account?.account_id.toString(),
		account_to_block.toString(),
	])) as Account[] // need to somehow place the account_to_block's account_id in there as well
	try {
		if (follow_row.length < 1) {
			await Query(blockQuery, [
				account?.account_id.toString(),
				account_to_block.toString(),
			])
			const check_follow_row_1 = (await Query(checkFollowQuery, [
				account?.account_id.toString(),
				account_to_block.toString(),
			])) as Account[]
			// If I'm following blocked account, unfollow them
			if (check_follow_row_1.length > 0) {
				await Query(unfollowQuery, [
					account?.account_id.toString(),
					account_to_block.toString(),
				])
			}
			// If blocked account following me, make them unfollow me
			const check_follow_row_2 = (await Query(checkFollowQuery, [
				account_to_block.toString(),
				account?.account_id.toString(),
			])) as Account[]
