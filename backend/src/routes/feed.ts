...
WHERE A.account_id IN (SELECT followed_account_id FROM account_followers WHERE account_id = ?) ORDER BY created_at
`
...
async function GetPosts(req: Request, res: Response) {
	...
	if(account === undefined) {
		...
		const posts = (await Query(feedQuery, [account.account_id.toString()])) as Post[]
		if (posts.length > 0) {
