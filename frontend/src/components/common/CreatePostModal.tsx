			...
			<Modal classNames={{ modal: "rounded-md w-4/5 h-4/5" }} open={open} showCloseIcon={false} onClose={onCloseModal} center>
				<div className="flex flex-col justify center items-center h-full">
					...
					<div className="w-full bg-gray-200 flex flex-col justify center items-center h-full">
						...
						<Form onSubmit={(data) => createMutation.mutate(data as unknown as CreateNewPost)}>
							<div className="px-8">
								...
								<br />
								{
									imgUploaded === true && (
										<>
											<label>Caption</label>
											<InputField type="text" name="caption" placeholder="Caption" required minLength={5} />
			
											<label>Location Name</label>
											<InputField type="text" name="location" placeholder="Location" required minLength={5} />
												
										
											<SubmitButton text="Submit" loading={createMutation.isLoading}/>
										</>
									)
								} 
